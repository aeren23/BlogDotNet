    ...
    public class ArticleService : IArticleService
    {
        ...
        public async Task<ArticleListDto> GetAllByPagingAsync(Guid? categoryId, int currentPage = 1, int pageSize = 3,bool isAscending = false)
        {
            ...
            var articles=categoryId==null
                ?await unitOfWork.GetRepository<Article>().GetAllAsync(a=>!a.IsDeleted,x=>x.Category,i=>i.Image)
        ...
        public async Task<ArticleDto> GetArticleByGuidWithCategoryNonDeletedAsync(Guid articleId)
        {
        ...
        public async Task<List<ArticleDto>> GetAllDeletedArticlesWithCategory()
        {
            var articles = await unitOfWork.GetRepository<Article>().GetAllAsync(x => x.IsDeleted, x => x.Category);
            var map = mapper.Map<List<ArticleDto>>(articles);
